...
async def add_messages(messages: Message | list[Message], flow_id: str | UUID | None = None):
...
async def update_messages(messages: Message | list[Message]) -> list[Message]:
    if not isinstance(messages, list):
        messages = [messages]
...
async def store_message(
    ...
    if hasattr(message, "id") and message.id:
        return await update_messages([message])
    return await add_messages([message], flow_id=flow_id)
