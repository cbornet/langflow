...
async def create_default_folder_if_it_doesnt_exist(session: AsyncSession, user_id: UUID):
    folder = (await session.exec(select(Folder).where(Folder.user_id == user_id))).first()
    if not folder:
        folder = Folder(
...
async def get_default_folder_id(session: AsyncSession, user_id: UUID):
    folder = (await session.exec(select(Folder).where(Folder.name == DEFAULT_FOLDER_NAME, Folder.user_id == user_id))).first()
    if not folder:
