...
async def setup_superuser(settings_service, session: AsyncSession) -> None:
    ...
    try:
        user = await get_or_create_super_user(session=session, username=username, password=password, is_default=is_default)
        if user is not None:
...
async def initialize_services(*, fix_migration: bool = False) -> None:
    ...
    await setup_superuser(get_service(ServiceType.SETTINGS_SERVICE), await anext(get_async_session()))
    try:
