...
async def created_messages(async_session):  # noqa: ARG001
    async with async_session_scope() as _session:
        ...
        messagetables = await add_messagetables(messagetables, _session)
...
async def test_get_messages():
    ...
    messages = await get_messages(sender="User", session_id="session_id2", limit=2)
...
async def test_add_messages():
    ...
    messages = await add_messages(message)
    assert len(messages) == 1
    assert messages[0].text == "New Test message"


@pytest.mark.usefixtures("client")
async def test_add_messagetables(async_session):
    messages = [MessageTable(text="New Test message", sender="User", sender_name="User", session_id="new_session_id")]
    added_messages = await add_messagetables(messages, async_session)
...
async def test_delete_messages():
    ...
    await add_messages([message])
    messages = await get_messages(sender="User", session_id=session_id)
    assert len(messages) == 1
...
async def test_astore_message():
    ...
    stored_messages = await get_messages(sender="User", session_id=session_id)
...
@pytest.mark.usefixtures("client")
async def test_update_single_message(created_message):
    # Modify the message
    created_message.text = "Updated message"
    updated = await update_messages(created_message)
...
@pytest.mark.usefixtures("client")
async def test_update_multiple_messages(created_messages):
    # Modify the messages
    for i, message in enumerate(created_messages):
        message.text = f"Updated message {i}"

    updated = await update_messages(created_messages)
...
async def test_update_nonexistent_message():
    ...
    updated = await update_messages(message)
    assert len(updated) == 0


@pytest.mark.usefixtures("client")
async def test_update_mixed_messages(created_messages):
    ...
    updated = await update_messages(messages_to_update)
...
@pytest.mark.usefixtures("client")
async def test_update_message_with_timestamp(created_message):
...
@pytest.mark.usefixtures("client")
async def test_update_multiple_messages_with_timestamps(created_messages):
...
@pytest.mark.usefixtures("client")
async def test_update_message_with_content_blocks(created_message):
...
@pytest.mark.usefixtures("client")
async def test_update_message_with_nested_properties(created_message):
